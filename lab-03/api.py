...
def rsa_decrypt():
    data = request.json
    ciphertext_hex = data.get('ciphertext') or data.get('cipher_text')
    key_type = data['key_type']
    private_key, public_key = rsa_cipher.load_keys()
    if key_type == 'public':
        key = public_key
    ...
    if not ciphertext_hex:
        return jsonify({'error': 'ciphertext is required'})

    ciphertext_hex = ciphertext_hex.strip().replace(" ", "").replace("\n", "").replace("\r", "")
    ...
    try:
        ...
        return jsonify({'error': 'failed to decrypt ciphertext with the selected key'}), 400
...
def rsa_verify_signature():
    ...
    signature_hex = signature_hex.strip().replace(" ", "").replace("\n", "").replace("\r", "")
